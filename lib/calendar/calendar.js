...
function initCalendarKeyboardNav(calendar, calculatorResources) {
  ...
  function initAriaStructure() {
    ...
    // Set up each visible month
    $el.find(".calendar-month").each(function () {
      const monthEl = this;
      monthEl.setAttribute("role", "grid");

      // Add row roles and set up columnheader for weekday names
      $$(monthEl)
        .find(".calendar-row")
        .each(function () {
          this.setAttribute("role", "row");

          // Set columnheader role on weekday name cells
          $$(this)
            .find(".calendar-week-day")
            .each(function () {
    ...
    // Set up day cells with roving tabindex
    ...
    // Also set up prev/next month days (but not as tab targets)
    $el
      .find(
    ...
    // Set up disabled days with gridcell role and aria-disabled
