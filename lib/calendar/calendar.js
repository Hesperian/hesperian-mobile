...
//import { attatchDateSelector} from './calendar-dropdown';
import './calendar.scss';
import MessageFormat from 'messageformat';
import {dayRangeFromNow, daysToToday} from './date-utils';
...
function initDatePicker(page) {
  ...
  <a href="#" class="link icon-only calendar-prev-month-button ripple-color-gray" role="button" aria-label="${previousMonth}">
    <i class="icon icon-prev ${iconColor}"></i>
  </a>
  <div><span class="current-month-value"></span>&nbsp;<span class="current-year-value"></span></div>
  <a href="#" class="link icon-only calendar-next-month-button ripple-color-gray" role="button" aria-label="${nextMonth}">
...
`.trim();
  ...
  function initCalendar() {
    myCalendar = page.app.calendar.create({
  ...
  function initJqueryCalendar(calendarType, language, localNumbers) {
    ...
    jQuery(function($) {
      ...
      $('.gestcalc-container').calendarsPicker({
        ...
        pickerClass: 'calendar-jq calendar calendar-jq-type-' + calendarType + ' jq-calendar-locale-' + language,
    ...
    function showDate(date) {
      var gregorianDate = jQuery.calendars.instance('gregorian').fromJD(date[0].toJD());
...
function initResults(page) {
  ...
  const timePregnantFormater = messageFormater.compile(calculatorResources.timepregnant);
  const timePerioidFormater = messageFormater.compile(calculatorResources.timeperiod || '');
  const resultstitleFormater = messageFormater.compile(calculatorResources.resultstitle);
  ...
  function updateDateRangeElements(elements, days) {
    ...
    for (i = 0; i < elements.length; i++) {
      ...
      if ((min === undefined || min <= days) && (max === undefined || max >= days)) {
        count++;
  ...
  const override = calculatorResources.timepregnantOverrides && calculatorResources.timepregnantOverrides[diffDays];

  const tp = override ? override : timePregnantFormater({
    "WEEKS": weeks,
    "DAYS": days
  });

  $$('#timepregnant', page.el).text(tp);
  const tperiod = timePerioidFormater({
    "WEEKS": weeks,
    "DAYS": days
  });

  $$('#timeperiod', page.el).text(tperiod);
...
$$(document).on('page:beforein', '.page[data-page="gestcalc"]', function(e, page) {
  ...
  if (lastChosenDate) {
    ...
    const calendar = page.app.calendar.get($calendar[0]);
    if( calendar) {
