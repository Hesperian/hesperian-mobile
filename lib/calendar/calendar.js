...
function initResults(page) {
  ...
  const timePregnantFormater = messageFormater.compile(
    calculatorResources.timepregnant
  );
  const timePerioidFormater = messageFormater.compile(
    calculatorResources.timeperiod || ""
  );
  ...
  const dueDate = formatDate(appConfig.locale(), diffDays);
  ...
  $$("#results-list-title", resultsDom).text(diffDays + " " + rt);
}
