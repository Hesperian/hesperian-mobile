...
function exitAnyFullscreen(playerState) {
  if (!playerState.stack.classList.contains('hm-fullscreen')) return;

  exitCssFullscreen(playerState.stack);
  playerState.player.removeClass('vjs-fullscreen');
  playerState.player.trigger('fullscreenchange');
}

function registerPlayerLifecycle(playerState) {
  activePlayers.add(playerState);
  playerState.player.on('dispose', () => {
    activePlayers.delete(playerState);
  });
}

function bindPageVideoCleanup(pageEl, videojs) {
  if (pageEl.dataset.hmVideoCleanupBound === 'true') return;

  const $pageEl = Dom7(pageEl);
  const onPageBeforeRemove = () => {
    pageEl.querySelectorAll('.hm-video-player').forEach((playerEl) => {
      const player = videojs.getPlayer(playerEl);
      if (player && !player.isDisposed()) {
        player.dispose();
      }
    });
    Dom7(document).off('page:beforeremove', $pageEl, onPageBeforeRemove);
    delete pageEl.dataset.hmVideoCleanupBound;
  };

  Dom7(document).on('page:beforeremove', $pageEl, onPageBeforeRemove);
  pageEl.dataset.hmVideoCleanupBound = 'true';
}

function initializeVideoLifecycle() {
  document.addEventListener('pause', () => {
    activePlayers.forEach((playerState) => {
      if (!playerState.stack.isConnected) return;
      exitAnyFullscreen(playerState);
...
export function initializeVideo() {
  ...
  document.addEventListener('page:init', async function (e) {
    ...
    bindPageVideoCleanup(e.detail.pageEl, videojs);
  ...
  videojsReady.then(videojs => {
    ...
    document.querySelectorAll('.page').forEach((pageEl) => bindPageVideoCleanup(pageEl, videojs));
...
function initMobileOverlay(player, stack, device) {
  ...
  function enterFullscreen() {
    ...
    // avoiding the range-request path entirely. Keep this Android-only so iOS
    // playback is not unnecessarily restarted on every fullscreen entry.
    const currentSrc = player.currentSrc();
    if (device.android && currentSrc) {
...
function initVideoInContainer(container, videojs) {
  ...
  players.forEach((videoEl) => {
    ...
    const playerState = { player, stack };

    registerPlayerLifecycle(playerState);

    player.on('loadedmetadata', function () {
      const w = player.videoWidth();
      if (w > 0) stack.style.maxWidth = w + 'px';
    ...
    player.ready(function () {
      ...
      initMobileOverlay(player, stack, device);
    });
  });
}
