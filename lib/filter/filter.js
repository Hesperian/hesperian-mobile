...
function updatePageFilter($el, filter) {
  ...
  window.app.api.pageStructureChanged();
}
  ...
  <ul>
    <li>
      <a class="item-link smart-select filter-menu"">
        ...
        <span class="filter-menu-content"><i class="icon f7-icons">menu</i><span class="filter-menu-text">{{filter}}</span></span>
      </a>
    </li>
  </ul>
</div>
...
function attachToFilterSelect($el, opts) {
  $el.find(".filter-menu").forEach(function () {
    const el = this;
    const select = window.app.smartSelect.create({
      el: el,
      closeOnSelect: true,
      openIn: "popup"
    });
    const target = opts.target;
    const defaultText = opts.context.all;
