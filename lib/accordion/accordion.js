...
function initializeAccordions(app) {
  $$(document).on("page:init", function (e) {
    ...
    $$(".accordion-item", pageEl).each(function() {
      ...
      // Add accessible chevron icon (aria-hidden so screen readers skip it)
      // This replaces the CSS ::before pseudo-element which was being read aloud
      const $inner = $trigger.find(".item-inner");
      if ($inner.length && !$inner.find(".accordion-chevron").length) {
        $inner.prepend('<span class="accordion-chevron" aria-hidden="true"></span>');
      }
    });
  });
  ...
  app.on("accordionOpened", function (el) {
    ...
    logEvent("accordionOpened", { "title": $trigger.text().trim() });
  });
  ...
  app.on("accordionClosed", function (el) {
    ...
    logEvent("accordionClosed", { "title": $trigger.text().trim() });
  });
}
