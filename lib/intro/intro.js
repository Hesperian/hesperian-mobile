...
function chooseLanguage(appData, appConfig) {
  const languageChooser = new Promise((resolve, reject) => {
    if (window.isWebsite || appConfig.get("languageHasBeenChosen")) {
      resolve();
      return;
    }
...
function createTour(app, appData) {
  ...
  function preparePanels(menu) {
    ["left", "right"].forEach((side) => {
      ...
      //F7 v7 requires a selector to identify the panel
      const panel = app.panel.get(`.panel.panel-${side}`);
      if (open && (!panel || !panel.opened)) {
        app.panel.open(`.panel.panel-${side}`);
      }

      if (!open && panel && panel.opened) {
        app.panel.close(`.panel.panel-${side}`);
      }
    });
  }
