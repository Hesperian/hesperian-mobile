...
function chooseLanguage(appData, nextStep) {
  if (languageChosen || window.isWebsite) {
    nextStep();
    return;
  }
  ...
  const dialog = window.app.dialog.create({
    ...
    content,
    buttons: [{ text: resources.get("settings.languages.continue") || "⮕", close: true }],
    on: {
      closed: function () {
        if(!languageChosen) {
          nextStep();
...
function initializeIntro(app, appConfig, resources, appData) {
    ...
    defaultStepOptions: {
      ...
      when: {
        show() {
          ...
          if (
            $(currentStepElement).data("shepherd-step-id") === "tour-intro"
          ) {
  ...
  app.$(document).on("page:afterin", '.page[data-page="index"]', function (_e) {
    chooseLanguage(appData, function () {
      if (isIntroDisabled()) {
        return;
      }
      setTimeout(function() {
