...
function createTour(app, appData) {
  ...
  // Ensure the panel state matches `menu` ("left", "right", or undefined for
  // neither). We act on each panel via its own instance so there is no
  // ambiguity between the desired-menu name and the panel being inspected.
  function preparePanels(menu) {
    ["left", "right"].forEach((side) => {
      const shouldBeOpen = menu === side;
      const panel = app.panel.get(side);
      if (!panel) return;

      if (shouldBeOpen && !panel.opened) {
        panel.open();
      } else if (!shouldBeOpen && panel.opened) {
        panel.close();
      }
    });
  }
