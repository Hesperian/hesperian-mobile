...
function chooseLanguage(appData, appConfig, nextStep) {
  if (appConfig.get('languageHasBeenChosen') || window.isWebsite) {
    nextStep();
    return;
  }
  ...
  stepsSpecs.forEach((spec, index) => {
    ...
    let step = {
      ...
      showOn() {
        ...
        function elementIsVisable($el) {
          if ($el.length === 0) {
            return false;
          }
          return $el.css("display") !== "none";
        }
        return !spec.element || elementIsVisable($(spec.element));
      },
...
function initializeIntro(app, appConfig, resources, appData) {
  ...
  app.$(document).on("page:afterin", '.page[data-page="index"]', function (_e) {
    ...
    // Language choice is, in effect,the first step of the tour
    chooseLanguage(appData, appConfig, function () {
      if (isIntroDisabled()) {
        return;
      }
      if (tour) {
        ...
        introDisabledConfig(true); // no lang next time if no tour
      }
    });
  });
}
