...
function isIntroNeeded() {
  if (introTourOverride !== null) {
    return introTourOverride;
  }
...
function chooseLanguage(appData, appConfig) {
  const languageChooser = new Promise((resolve, reject) => {
    ...
    // Skip the language dialog if:
    // 1. Tour is explicitly disabled (introTourOverride === false), OR
    // 2. Tour is not explicitly enabled AND language has already been chosen
    if (introTourOverride === false || 
        ((introTourOverride !== true) && appConfig.get("languageHasBeenChosen"))) {
      resolve();
      return;
    }
...
function createTour(app, appData) {
  ...
  ["complete", "cancel"].forEach((eventName) => {
    tour.on(eventName, handleTourFinished);
  });
