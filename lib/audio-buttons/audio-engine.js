...
/**
 * Creates an audio engine that wraps the Cordova TTS plugin (window.TTS).
 *
 * The native plugin (cordova-plugin-tts-advanced feat/updates branch) now
 * correctly handles callbacks on both platforms:
 * - iOS: didCancelSpeechUtterance: resolves cancelled speak callbacks with
 *   error, stop: clears stale callback state and sends its own result.
 * - Android: utterance ID is passed to tts.speak() so onDone resolves the
 *   correct callback, stop() calls callbackContext.success().
 *
 * Each speak's native callback resolves only its own closure-captured promise.
 * Both resolve and reject settle the promise (cancellation is not an error
 * from the app's perspective — it just means speech is done).
 */
function createCordovaTTSEngine() {
  var tts = window.TTS;

  return {
    speak: function (context) {
      return new Promise(function (resolve) {
        tts.speak(context).then(resolve, resolve);
      });
    },
    stop: function () {
      return tts.stop();
...
function initAudioEngine(forceAudio) {
    if (window.TTS) {
        audioEngine = createCordovaTTSEngine();
