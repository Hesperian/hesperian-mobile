...
function hmTTS() {
  hmTTSRate = appConfig.get("hmTTSRate") || "1.0";
  this.tts = window.TTS;
  this.lastTapTime = 0;
  this.lastTapElement;
  this.enabled = false;
...
hmTTS.prototype.convertBlockToText = function (block) {
  ...
  const linkPreText = `<span>${resources.get(
  ...
  const substitutions = resources.get("audio.substitutions");
...
hmTTS.prototype.showConfigSheet = function () {
  ...
  const audioStrings = resources.get("audio.strings");
  ...
  var configSheet = app.sheet.create({
  ...
  var popover = app.popover.create({
    ...
    content: `<div class="popover hm-audio-popover">
      <div class="popover-inner popover-close">
        ...
        <div class="block">
        <a><span class="popover-play-link"><i class="material-icons">${playIcon}</i><span class="popover-play-caption">${resources.get(
...
hmTTS.prototype.speak = function (context) {
  ...
  if (!context.locale) {
    const updatedLocale = self.bestLocaleForLanguage(appConfig.locale());
    if (updatedLocale) {
