...
import { initAudioEngine, getAudioEngine } from "./audio-engine";
import { speak, stop, ttsIsPlaying } from "./audio";
import { getApp } from "../app";
import { setVoiceOverFocus, isActivationKey, KEYS, isEscapeKey, createFocusTrapHandler } from "../accessibility/accessibility";
...
function enableToolbar(enable, $pageEl, toolbar) {
  if (enable) {
    toolbar.addClass("hm-audio-enabled");
  } else {
    toolbar.removeClass("hm-audio-enabled");
  }  
}
...
hmTTS.prototype.resetPage = function (pageEl) {
  ...
  this.enabled =
  audioEnabledLocale(this.initSpec) && getAudioEngine() && this.audioBlocks.length;

  enableToolbar(this.enabled, this.$pageEl, this.toolbar);
};
...
hmTTS.prototype.attachToPage = function (page, initSpec) {
  ...
  $$(".hm-audio-block", self.pageEl).each(function() {
    const $block = $$(this);
    const tagName = this.tagName.toLowerCase();
    // For lists (ul/ol), prepend to the first li to avoid invalid HTML structure
    if (tagName === 'ul' || tagName === 'ol') {
      const $children = $block.children('li');
      if ($children.length) {
        $$($children[0]).prepend('<div class="hm-audio-block-item-top"></div>');
      }
    } else {
      $block.prepend('<div class="hm-audio-block-item-top"></div>');
    }
  });

  $$(".hm-audio-block", self.pageEl).on("click", function (e) {
    if( !self.pageEl.classList.contains("hm-audio-toolbar-visible")) {
      return;
    }

    const timeElapsed = new Date().getTime() - self.lastTapTime;
    if (timeElapsed < 300) {
      const blockIndex = self.audioBlocks.indexOf(this);
  ...
  $$(document).on("page:beforeout", self.$pageEl, function (_e) {
    if (self.isEnabled() && getAudioEngine()) {
      self.stopBlock();
    }
  });
...
hmTTS.prototype.convertBlockToText = function (block) {
  ...
  const linkPreText = `<span>${this.initSpec.resources.get(
    "common.audio.strings.clickhere"
  )} </span>`;
  findIncludingRoot(
    $blockEl,
  ...
  const substitutions = this.initSpec.resources.get("common.audio.substitutions");
...
hmTTS.prototype.showAudioToolbar = function (show, animate) {
  ...
  const returnFocus = audioToolbarLastShown;
  ...
  if (show) {
    ...
    this.$pageEl.addClass("hm-audio-toolbar-visible");
    ...
    this.$pageEl.removeClass("hm-audio-toolbar-visible");
...
hmTTS.prototype.showConfigSheet = function () {
  ...
  const audioStrings = this.initSpec.resources.get("common.audio.strings");
  const closeLabel = this.initSpec.resources.get("common.audio.ariaLabels.close") || "Close";
    ...
    <div class="sheet-modal-inner">
      <div class="page-content">
        <div class="list hm-audio-rate-list" role="radiogroup" aria-labelledby="hm-audio-rate-title">
          <ul>
            <li><a href="#" class="button hm-audio-rateoption-button aria-keyboard-accessible ${getActiveClass(
    "0.25"
  )}" role="radio" aria-checked="${getAriaChecked("0.25")}" data-hm-audio-rate="0.25">0.25x <span class="hm-rateoption-caption">(${audioStrings["slowspeedcaption"]
    })</span></a></li>
            <li><a href="#" class="button hm-audio-rateoption-button aria-keyboard-accessible ${getActiveClass(
      "0.5"
    )}" role="radio" aria-checked="${getAriaChecked("0.5")}" data-hm-audio-rate="0.5">0.5x</a></li>
            <li><a href="#" class="button hm-audio-rateoption-button aria-keyboard-accessible ${getActiveClass(
      "0.75"
    )}" role="radio" aria-checked="${getAriaChecked("0.75")}" data-hm-audio-rate="0.75">0.75x</a></li>
            <li><a href="#" class="button hm-audio-rateoption-button aria-keyboard-accessible ${getActiveClass(
      "1.0"
    )}" role="radio" aria-checked="${getAriaChecked("1.0")}" data-hm-audio-rate="1.0">1x</a></li>
            <li><a href="#" class="button hm-audio-rateoption-button aria-keyboard-accessible ${getActiveClass(
      "1.25"
    )}" role="radio" aria-checked="${getAriaChecked("1.25")}" data-hm-audio-rate="1.25">1.25x</a></li>
            <li><a href="#" class="button hm-audio-rateoption-button aria-keyboard-accessible ${getActiveClass(
      "1.5"
    )}" role="radio" aria-checked="${getAriaChecked("1.5")}" data-hm-audio-rate="1.5">1.5x</a></li>
            <li><a href="#" class="button hm-audio-rateoption-button aria-keyboard-accessible ${getActiveClass(
      "1.75"
    )}" role="radio" aria-checked="${getAriaChecked("1.75")}" data-hm-audio-rate="1.75">1.75x <span class="hm-rateoption-caption">(${audioStrings["fastspeedcaption"]
    })</span></a></li>
          </ul>
        </div>
      </div>
  ...
  let configSheet = app.sheet.create({
    ...
    on: {
      open: function (sheet) {
        ...
        $el.on("keydown", createFocusTrapHandler(el => el.querySelector('.sheet-close')));
      },
...
hmTTS.prototype.showPopOver = function (blockIndex, targetEl) {
  ...
  const playIcon = "&#xe039;";
  const closeLabel = this.initSpec.resources.get("common.audio.ariaLabels.close") || "Close";
  const dialogLabel = this.initSpec.resources.get("common.audio.ariaLabels.playFromHereDialog") || "Play from here";
  ...
  var popover = app.popover.create({
    ...
    content: `<div class="popover hm-audio-popover" role="dialog" aria-modal="true" aria-label="${dialogLabel}">
      <div class="popover-inner popover-close">
        ...
        <div class="block">
        <a role="button" class="popover-play-link-wrapper aria-keyboard-accessible"><span class="popover-play-link"><i class="material-icons" aria-hidden="true">${playIcon}</i><span class="popover-play-caption">${this.initSpec.resources.get(
      "common.audio.strings.playfromhere"
    )}</span></span></a>
        </div>
      </div>
      </div>`,
    ...
    on: {
      open: function (popover) {
        ...
        // Focus trap with Escape to close
        $el.on("keydown", createFocusTrapHandler(el => el.querySelector('.hm-audio-popover-close')));
      },
...
hmTTS.prototype.speak = function (context) {
  ...
  if (!context.locale) {
    const appLocale = this.initSpec.appConfig.locale();
    if (getAudioEngine() === window.TTS) {
      // Cordova plugin can auto-detect a more specific locale (e.g. es-MX vs es-XL)
      // when the device language matches, so only override when they differ.
      const updatedLocale = self.bestLocaleForLanguage(appLocale);
      if (updatedLocale) {
        context.locale = updatedLocale;
      }
    } else {
      // Web Speech API always needs an explicit locale for voice selection.
...
hmTTS.prototype.pauseBlock = function () {
  if (this.audioToolbar) {
    ...
    this.announceStatus("common.audio.ariaLabels.statusPaused");
  }
  stop();
};
...
hmTTS.prototype.stopBlock = function () {
  ...
  if (this.audioToolbar) {
    ...
    this.announceStatus("common.audio.ariaLabels.statusStopped");
  }
  this.pauseBlock();
};
...
hmTTS.prototype.focusOnBlock = function (block) {
  ...
  function scrollToBlock() {
    // Use find() instead of children() to handle lists where the sentinel is inside the first li
    const $foundSentinels = $$(block).find(".hm-audio-block-item-top");
    if ($foundSentinels.length) {
      const scrollTopSentinel = $foundSentinels[0];
...
hmTTS.prototype.playBlock = function () {
  ...
  this.announceStatus("common.audio.ariaLabels.statusPlaying");
...
function initAudioButtons(initSpec, forceAudio) {
  initAudioEngine(forceAudio);
