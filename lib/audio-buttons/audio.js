...
let speakingID = 0; // We will track a unique id for each speak() for (at least) debugging purposes.
...
function normalizeRate(rate) {
  const app = getApp();
  if (app && app.device && app.device.ios) {
...
function checkSpeechQueue() {
  ...
  if (nowPlaying.mode === MODE_STARTING) {
    ...
    const currentSpeakingID = nowPlaying.speakingID;
    getAudioEngine().speak(nowPlaying.context).finally(
      function() {
        doSpeechCommandCompleted(currentSpeakingID);
      },
      function() {
        doSpeechCommandCompleted(currentSpeakingID);
      },
...
// expectedSpeakingID guards against stale completions (e.g. iOS resolving
// an old callback after a stop→speak sequence).
function doSpeechCommandCompleted(expectedSpeakingID) {
  if (!nowPlaying) return;
  if (nowPlaying.speakingID !== expectedSpeakingID) return;

  const eventContext = {
    speakingID: nowPlaying.speakingID,
  ...
  const app = getApp();
  if (app) {
    app.emit('speechStopped', eventContext);
  }
}


// Worker for starting a speak()
// Enqueues the context, and delays a tick to checkSpeechQueue()
...
function doSpeechCancelled() {
  // Stop the speak. 
  // Note: While the cordova interface to the plugin accepts a promise, in fact it's never
  // resolved or rejected. The plugin seems to assume immediate effect.
  if(!getAudioEngine()) return;
  
  getAudioEngine().stop().then(
    function() {
    },
    function() {
    }
