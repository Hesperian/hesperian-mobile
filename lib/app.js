...
// ensure that our custom CSS loads after the Framework 7 CSS
import './css/app.scss';
...
import { initializeSidePanels, updateSidePanels } from './sidepanels/sidepanels';
...
function createApp(config) {
  ...
  //TODO confirm if the below iPhone specific code is still needed, the earlier variant of it was removed in the f7upgrade branch
...
app.on('pageInit', function(page) {
    ...
    let searchbar = createSearchBar(app, page.$pageEl, resources);
    let searchList = createSearchList(app, page.$pageEl, resources);

    $$(document).on('page:afterout', page.$pageEl, function(_e) {
      // searchbar.clear();
      searchbar.disable();
    });

    $$(document).on('page:beforeremove', page.$pageEl, function(_e) {
      searchbar.destroy();
      searchbar = null;
    ...
    page.$pageEl.on('click', 'a.self', function (e) {
  ...
  initializeIntro(app, appConfig, resources, config.appData.intro)
  ...
  */
  ...
  app.el.dispatchEvent(new CustomEvent('appInit', {bubbles: true, detail: app}));
