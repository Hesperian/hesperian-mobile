...
function configureApp(app, config, $$, isWebsite, setLangUrl) {
  let initialLang;
  let introTourOverride = null; // Derived from ?tour= query param when available.\

  const queryParams = app.utils.parseUrlQuery(window.location.href);

  if (typeof queryParams.tour !== "undefined") {
    const normalizedTour = String(queryParams.tour)
      .trim()
      .toLowerCase();
    if (normalizedTour === "true") {
      introTourOverride = true;
    } else if (normalizedTour === "false") {
      introTourOverride = false;
    }
  }
