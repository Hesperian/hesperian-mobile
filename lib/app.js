...
function createApp(config) {
  ...
  // Dev helper to confirm the app boot sequence executes.
  console.log("Framework7 app initialised", {
    version: Framework7.version,
    el: conf.el || conf.root,
  });
  ...
  console.log("Main view created", {
    ...
    path: mainView.router?.currentRoute?.path,
  });

  if (mainView.router && typeof mainView.router.on === "function") {
    mainView.router.on("routeChange", function routeChange(newRoute, previousRoute) {
      console.log("router:routeChange", {
        url: newRoute && newRoute.url,
        path: newRoute && newRoute.path,
        name: newRoute && newRoute.name,
        from: previousRoute && previousRoute.url,
      });
    });
  }
