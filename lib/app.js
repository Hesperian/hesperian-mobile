...
import { setPageFocus, announceNavigation, injectAriaIntoElement } from "./accessibility/accessibility";
...
function configureApp(app, config, $$, isWebsite, setLangUrl) {
  ...
  function applySmartSelectPopupCloseIcon(smartSelect) {
    if (!smartSelect || smartSelect.params?.openIn !== "popup") {
      return;
    }
    ...
    const closeLabel =
      smartSelect.params.popupCloseLinkText ||
      resources.get("common.dialog.close") ||
      "Close";

    $popup.find(".popup-close").each(function () {
      const $close = $$(this);
      $close.attr("aria-label", closeLabel);
      $close.empty();
      $close.append(
  ...
  localizations.forEach((locale) => {
    ...
    localeDirMap[code] = locale.language_dir || "ltr";
    let loc = {
      ...locale,
    };
  ...
  // Switch between LTR and RTL Framework7 stylesheets.
  // Both <link> tags are present in index.html; the inactive one uses
  // media="not all" so it downloads but does not apply.
  function setF7CssDirection(dir) {
    const ltrLink = document.getElementById("f7-css-ltr");
    const rtlLink = document.getElementById("f7-css-rtl");
    if (!ltrLink || !rtlLink) return;
    if (dir === "rtl") {
  ...
  function setLocale(locale, pageEl) {
    appConfig.locale(locale);
    document.documentElement.lang = locale;
    document.documentElement.dir = localeDirMap[locale] || "ltr";
    setF7CssDirection(localeDirMap[locale] || "ltr");
    const smartSelectCloseText =
      resources.get("common.dialog.close") || "Close";
    if (app.params?.smartSelect) {
      app.params.smartSelect.popupCloseLinkText = smartSelectCloseText;
    }
    if (isWebsite) {
      setLangUrl(locale);
    }
    ...
    if (pageEl) {
      injectAriaIntoElement(pageEl);
    }
  }

  setLocale(appConfig.locale());

  app.on("smartSelectOpen", function (smartSelect) {
    applySmartSelectPopupCloseIcon(smartSelect);
  });

  app.on("smartSelectOpened", function (smartSelect) {
    applySmartSelectPopupCloseIcon(smartSelect);
  });
  ...
  $$(document).on("page:init", ".page", function (e) {
    ...
    $$('[data-bookmark-for]', page.el).each(function () {
      const bookmarkLabel = resources.get('common.bookmarkAriaLabel');
  ...
  $$(document).on("page:afterin", function (e) {
    ...
    // Announce the page change to screen readers and move programmatic focus
    // to the page heading. Uses a persistent aria-live region (announceNavigation)
    // for broad screen reader coverage (VoiceOver, TalkBack, NVDA, JAWS) plus a
    // single preventScroll focus call — replacing the 10x setInterval hack that
    // caused visible viewport jitter on Android WebView.
    const pageTitle = $$(page.el).data("title") || "";
    announceNavigation(pageTitle);
    // Defer focus briefly so F7 finishes all post-transition DOM work.
    // Target the page h1 heading for best WCAG focus order compliance.
    // Fall back to .page-content which always has tabindex="0" from page:init.
    setTimeout(() => {
      var focusTarget = document.querySelector(".page-current h1")
          || document.querySelector(".page-current .page-content");
      if (focusTarget) {
        setPageFocus(focusTarget);
      }
    }, 50);
  });
  ...
  app.api = {
    ...
    pageStructureChanged: function () {
      if (app.views) {
        const currentPage = app.views?.main.router.currentPageEl;
        injectAriaIntoElement(currentPage);
