...
function createSearchBar(app, $pageEl, resources) {
...
function createLocalSearchBar(app, $pageEl, resources) {
  let searchbar = app.searchbar.create({
    ...
    on: {
      ...
      search: function (searchbar, query, previousQuery) {
        if (query) {
          $$("#app").addClass("local-search-in");
        } else {
          $$("#app").removeClass("local-search-in");
        }
      },
    },
  });
...
function createLocalSearchList(app, $pageEl, resources) {
  ...
  const pageTitle = $pageEl.data("title").toLowerCase();
 // const trimmer = new RegExp(`^${escapeRegExp(pageTitle)}.\s*`, "i");
