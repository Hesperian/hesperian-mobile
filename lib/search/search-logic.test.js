...
describe("parseQuery", () => {
  ...
  it("tokenizes Arabic words instead of dropping them", () => {
    // `\w` is ASCII-only; the regression we're guarding against tokenized
    // every Arabic string to [] and broke search entirely.
    expect(parseQuery("الحمل")).toEqual(["الحمل"]);
    expect(parseQuery("علامات الخطر أثناء الحمل")).toEqual([
      "علامات",
      "الخطر",
      "اثناء",
      "الحمل",
  ...
  it("folds Arabic alef variants (أ آ إ → ا) via NFD + combining-mark strip", () => {
    // All four queries should normalize to the same token.
    expect(parseQuery("أم")).toEqual(parseQuery("ام"));
    expect(parseQuery("آم")).toEqual(parseQuery("ام"));
    expect(parseQuery("إم")).toEqual(parseQuery("ام"));
  });

  it("folds Arabic ya/alef-maqsura and ta-marbuta", () => {
    expect(parseQuery("إلى")).toEqual(parseQuery("الي"));
    expect(parseQuery("كرة")).toEqual(parseQuery("كره"));
  });

  it("strips Arabic tashkeel (harakat) and tatweel", () => {
    // Fatha, shadda, kasra, etc. should not affect matching.
    expect(parseQuery("كِتَابٌ")).toEqual(parseQuery("كتاب"));
    // Tatweel is a decorative letter-joining character.
    expect(parseQuery("كتـــاب")).toEqual(parseQuery("كتاب"));
  });

  it("splits on Arabic punctuation (comma U+060C, semicolon U+061B, question mark U+061F)", () => {
    // Note: tokens are returned in their normalized form, so ة folds to ه.
    expect(parseQuery("الحمل،الولادة")).toEqual(["الحمل", "الولاده"]);
    expect(parseQuery("ما هذا؟")).toEqual(["ما", "هذا"]);
  });
});
...
describe("createSearchItem", () => {
  ...
  it("includes searchTitle when present in source", () => {
    const source = {
      title: "Very Long Title",
      searchTitle: "Short",
      route: "/pages/test",
    };
    const item = createSearchItem(source);
    expect(item.searchTitle).toBe("Short");
    expect(item.title).toBe("Very Long Title");
  });

  it("omits searchTitle when not in source", () => {
    const source = { title: "Normal", route: "/pages/test" };
    const item = createSearchItem(source);
    expect(item.searchTitle).toBeUndefined();
  });
});
...
describe("displayTitle", () => {
  it("returns searchTitle when present", () => {
    expect(displayTitle({ title: "Long Title", searchTitle: "Short" })).toBe(
      "Short"
    );
  });

  it("falls back to title when searchTitle is absent", () => {
    expect(displayTitle({ title: "Normal Title" })).toBe("Normal Title");
  });
});
