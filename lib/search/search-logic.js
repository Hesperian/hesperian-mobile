 ...
 * Splits on anything that is not a Unicode letter, digit, or underscore.
 * Note: `\w` is ASCII-only in JavaScript regex, so we use Unicode property
 * escapes `\p{L}` and `\p{N}` (with the /u flag) so non-Latin scripts like
 * Arabic tokenize correctly.
 * Each word is normalized (lowercase, accents and Arabic letter variants
 * folded) by searchNormalForm. Empty tokens are filtered out.
 */
